...
public class ArchivoValidator {
    ...
    public static void validate( String nombreDocumento, byte[] archivo) {
        ...
        String extension = nombreDocumento.contains(".") ? nombreDocumento.substring(nombreDocumento.lastIndexOf('.') + 1).toLowerCase() : "";
        if (!extensionesPermitidas.contains(extension)) throw new InvalidArgumentException("Extension no permitida");
    }
}
